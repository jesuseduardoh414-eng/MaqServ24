...
export function estadoCotizacion(q: EntradaVigencia, hoy: Date = new Date()): QuoteState {
  ...
  return soloFecha(q.validUntil) >= soloFecha(hoy) ? 'vigente' : 'vencida';
}
...
export function diasParaVencer(validUntil: Date | null, hoy: Date = new Date()): number | null {
  ...
  const b = Date.parse(`${soloFecha(hoy)}T00:00:00Z`);
  return Math.round((a - b) / 86_400_000);
}

/** Fecha de vigencia por defecto al responder, en formato ISO corto. */
